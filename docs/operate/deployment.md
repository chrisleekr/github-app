...
| Stage         | Base             | Purpose                                                                                                                                                                                                                                         |
| ------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base`        | `oven/bun:1.4.2` | Installs Node.js from NodeSource (major pinned in the Dockerfile, currently 25) for the Claude Code CLI, npm 11, `curl`, `git`, `@anthropic-ai/claude-code` globally (pinned via the `CLAUDE_CODE_VERSION` ARG), plus targeted OS CVE upgrades. |
...
| `CLAUDE_CODE_VERSION` | pinned in the Dockerfile, never passed by CI | Global `@anthropic-ai/claude-code` CLI pin. Must equal the `claudeCodeVersion` of the installed `@anthropic-ai/claude-agent-sdk`, because `CLAUDE_CODE_PATH` makes the SDK drive this CLI rather than its bundled one; `check:claude-code-version` fails CI on a mismatch. Carries a `# renovate:` marker, and `renovate.json` groups it with the SDK so both move in one PR. Deliberately not restated as a literal here: Renovate moves it and no CI gate compares this page against the Dockerfile. |
...
| -------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `@anthropic-ai/sdk`  | `0.128.0` | No advisory requires it: GHSA-5474-4w2j-mq4c / CVE-2026-34451 (fixed 0.81.0) and GHSA-p7fg-763f-g4gf / CVE-2026-41686 (fixed 0.91.1) are already excluded by both parents. Reason not recorded when pinned; held for a single deduplicated copy shared by `@anthropic-ai/claude-agent-sdk` (peer `>=0.93.0`) and `@anthropic-ai/bedrock-sdk` (`>=0.115.1 <1`). | A review confirms no advisory needs it.                             |
| `adm-zip`            | `0.6.1`   | GHSA-7q85-xj36-vmfc / CVE-2026-77301 memory-allocation DoS (fixed 0.6.1) and GHSA-vwc7-r8mq-g2x9 / CVE-2026-76845 symlink overwrite (affects 0.5.9 to 0.6.0, no patched version listed). Reached via `onnxruntime-node` (`^0.6.0`).                                                                                                                            | `onnxruntime-node` declares `adm-zip >= 0.6.1`.                     |
| `express-rate-limit` | `8.7.0`   | GHSA-46wh-pxpv-q5gq / CVE-2026-30827 IPv4-mapped IPv6 rate-limit bypass (fixed 8.2.2). Reached via `@modelcontextprotocol/sdk` (`^8.2.1`).                                                                                                                                                                                                                     | `@modelcontextprotocol/sdk` declares `express-rate-limit >= 8.2.2`. |
| `fast-uri`           | `3.1.8`   | GHSA-5jgf-p345-68v8 / CVE-2026-75931, GHSA-f65p-4m7j-42xc / CVE-2026-75975, GHSA-fph4-wmhf-6fwf / CVE-2026-75899, GHSA-jqff-g426-hqxp / CVE-2026-76172 host confusion and SSRF (fixed 3.1.6), plus earlier 3.x fixes. Reached via `ajv` (`^3.0.1`), three copies under `@modelcontextprotocol/sdk`, `ajv-formats` and `@commitlint/config-validator`.          | `ajv` declares `fast-uri >= 3.1.6`.                                 |
| `fast-xml-parser`    | `5.11.1`  | GHSA-gh4j-gqv2-49f6 / CVE-2026-41650 XMLBuilder comment and CDATA injection (fixed 5.7.0) and GHSA-8r6m-32jq-jx6q / CVE-2026-73569 repeated-DOCTYPE entity-limit reset (fixed 5.10.1). Reached via `@aws-sdk/xml-builder` (exact `5.5.8`).                                                                                                                     | `@aws-sdk/xml-builder` declares `fast-xml-parser >= 5.10.1`.        |
| `hono`               | `4.13.9`  | GHSA-gqvv-2mrq-wpjv / CVE-2026-84365, GHSA-g6gw-c38x-mqfc / CVE-2026-84364, GHSA-crvj-82cr-hjcx / CVE-2026-84363 (fixed 4.13.5), plus earlier 4.12.x fixes. Reached via `@modelcontextprotocol/sdk` (`^4.11.4`) and `@hono/node-server` (peer `^4`).                                                                                                           | Both parents declare `hono >= 4.13.5`.                              |
| `ip-address`         | `10.7.2`  | CVE-2026-69192 SSRF (fixed 10.3.1). Reached via `express-rate-limit` (`^10.2.0`) and `socks` (`^10.0.1`), which resolved two separate copies before the pin.                                                                                                                                                                                                   | Both parents declare `ip-address >= 10.3.1`.                        |
