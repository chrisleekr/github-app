...
The six runner quantities accept a narrower grammar than Kubernetes, and only in canonical form: CPU as whole cores or millicores (`2`, `500m`), memory and ephemeral storage as `Mi` or `Gi` (`512Mi`, `8Gi`). `4G`, `1.5` and `512Ki` are rejected, and so are `8192Mi` and `2000m`, because the API server re-serializes those as `8Gi` and `2`, while the spawner compares the returned Pod's resource strings byte for byte. A request above its matching limit is rejected too. Every one of these fails at startup naming the variable, rather than at Pod creation naming Pod identity.
...
Selects the system/user prompt split the agent executor passes to the Claude Agent SDK. See `src/config.ts:732#promptCacheLayout` for the Zod definition and `src/core/executor.ts:231#useCacheableLayout` for the runtime guard.
