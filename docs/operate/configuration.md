...
| `DEFAULT_MAXTURNS`            | unset                        | Process-wide turn cap. Set only if ops needs a hard ceiling. It reaches the agent as the job payload's `maxTurns`, which the workflow rail previously ignored, so before the Gate-2 wiring this value only took effect on the direct-pipeline rail. It now reaches the `review`, `resolve`, `implement`, and `remember` handlers too. `AGENT_MAX_TURNS` is unchanged: the executor still falls back to it when a job carries no cap (`src/core/executor.ts:361#resolvedMaxTurns`). |
...
| `WORKFLOW_RUNNER_CAPABILITY_SECRET`          | _none_                                     | Controller-only HMAC root for deadline-bound, per-attempt runner capabilities. Required on the controller, minimum 32 characters. It must differ from both daemon-auth slots, and setting it on a worker is rejected at boot.                                                  |
...
| `WORKFLOW_RUNNER_CPU_REQUEST`       | `500m`                                                      | CPU the runner Pod reserves. Must match `runnerCpuRequest` in the runner boundary ConfigMap.                                                                                                                                                                                                                                                                                                                   |
| `WORKFLOW_RUNNER_MEMORY_REQUEST`    | `1Gi`                                                       | Memory the runner Pod reserves. Must match `runnerMemoryRequest` in the runner boundary ConfigMap.                                                                                                                                                                                                                                                                                                             |
| `WORKFLOW_RUNNER_STORAGE_REQUEST`   | `2Gi`                                                       | Ephemeral storage the runner Pod reserves. Must match `runnerStorageRequest` in the runner boundary ConfigMap.                                                                                                                                                                                                                                                                                                 |
| `WORKFLOW_RUNNER_CPU_LIMIT`         | `2`                                                         | CPU ceiling. Exceeding it throttles the runner rather than killing it. Must match `runnerCpuLimit` in the runner boundary ConfigMap.                                                                                                                                                                                                                                                                           |
| `WORKFLOW_RUNNER_MEMORY_LIMIT`      | `4Gi`                                                       | Memory ceiling. Exceeding it OOMKills the runner mid-attempt, which surfaces as an expired lease unless the Pod post-mortem is read. Must match `runnerMemoryLimit` in the runner boundary ConfigMap.                                                                                                                                                                                                          |
| `WORKFLOW_RUNNER_STORAGE_LIMIT`     | `10Gi`                                                      | Ephemeral storage ceiling, and the workspace `emptyDir` `sizeLimit`. Must match `runnerStorageLimit` in the runner boundary ConfigMap.                                                                                                                                                                                                                                                                         |

The six runner quantities accept a narrower grammar than Kubernetes, and only in canonical form: CPU as whole cores or millicores (`2`, `500m`), memory and ephemeral storage as `Mi` or `Gi` (`512Mi`, `8Gi`). `4G`, `1.5` and `512Ki` are rejected, and so are `8192Mi`, `2000m` and `1000`, because the API server re-serializes those as `8Gi`, `2` and `1k`, while the spawner compares the returned Pod's resource strings byte for byte. A request above its matching limit is rejected too. Every one of these fails at startup naming the variable, rather than at Pod creation naming Pod identity.
...
Selects the system/user prompt split the agent executor passes to the Claude Agent SDK. See `src/config.ts:763#promptCacheLayout` for the Zod definition and `src/core/executor.ts:226#useCacheableLayout` for the runtime guard.
...
| Role                                     | Required                                                                                                                                                                                                                                      |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Controller (webhook server)              | GitHub App credentials, one AI provider credential, `VALKEY_URL`, `DATABASE_URL`, `DAEMON_AUTH_TOKEN`, and `WORKFLOW_RUNNER_CAPABILITY_SECRET`; K8s API access, `DAEMON_IMAGE`, and a WSS `ORCHESTRATOR_PUBLIC_URL` for structured workflows. |
