...
The startup lease is claimed before the Pod exists, so scheduling, image pull and volume setup all consume it while no heartbeat can renew it. Each reconciler pass therefore reads the Pod back and acts on what kubelet reports. Without this, a cold pull of the daemon image slower than the startup lease expired a healthy attempt that had not executed a line.

A Pod that is still coming up extends the startup lease by `WORKFLOW_RUNNER_STARTUP_LEASE_MS`. That covers `Unschedulable`, `ImagePullBackOff`, `CreateContainerConfigError` and a Pod with no status at all: each is routinely transient, and `CreateContainerConfigError` is partly self-inflicted, since the capability Secret is created after the Pod because it needs the Pod UID as its owner reference. Extension is bounded three ways: by the immutable attempt deadline, by `runner_payload_issued_at` being unset (so it can only prolong an attempt that has provably cloned nothing and written nothing), and by `WORKFLOW_RUNNER_STARTUP_BUDGET_MS` (15 minutes), measured from the Pod's own `creationTimestamp`. The budget matters because a Pod holding a lease also holds one of `MAX_CONCURRENT_REQUESTS`, and an unschedulable Pod is produced by exactly the full-cluster condition where those slots are scarcest.

A Pod is terminalized on the pass rather than waiting out the budget only for `InvalidImageName` and `ErrImageNeverPull`, which no retry resolves, or phase `Failed`. Past the budget the attempt fails with `not ready within 900s (<blocker>)`, naming the waiting reason or the `PodScheduled` condition. All of that applies only while the payload is unissued: once the runner holds its credential it may have pushed commits, so a dead Pod is left to lease expiry, whose notice tells the reader to inspect the repository. The reconcile log line carries `startupPhase` and, when stalled, `startupReason`; a refused extension logs `Workflow runner startup lease extension refused`.
