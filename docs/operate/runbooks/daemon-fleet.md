...
The startup lease is claimed before the Pod exists, so scheduling, image pull and volume setup all consume it while no heartbeat can renew it. Each reconciler pass therefore reads the Pod back and acts on what kubelet reports. A Pod that is still coming up with no error extends the startup lease by `WORKFLOW_RUNNER_STARTUP_LEASE_MS`, bounded by the immutable attempt deadline and refused once `runner_payload_issued_at` is set, so extension can only ever prolong an attempt that has provably cloned nothing and written nothing. A Pod whose container is waiting on `ErrImagePull`, `ImagePullBackOff`, `InvalidImageName`, `ErrImageNeverPull`, `CreateContainerConfigError` or `CreateContainerError`, or that has reached `Failed`, is terminalized on that pass instead of burning the remaining lease. The reconcile log line carries `startupPhase` and, when stalled, `startupReason`. Without this, a cold pull of the daemon image slower than the startup lease expired a healthy attempt that had not executed a line.
