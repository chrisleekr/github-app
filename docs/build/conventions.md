...
- **Bun version.** Pinned in `.tool-versions` (`bun 1.4.1`). All workflows use `oven-sh/setup-bun` with `bun-version-file: .tool-versions`. `.gitlab-ci.yml`'s `oven/bun:<ver>` pins are also verified by `check:docs-versions`.
