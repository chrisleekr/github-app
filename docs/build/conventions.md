...
- **Runtime.** Bun for the application; Node.js 25 for the Claude Code CLI subprocess. Both are installed in the Docker `base` stage.
- **Bun version.** Pinned in `.tool-versions` (`bun 1.4.2`). All workflows use `oven-sh/setup-bun` with `bun-version-file: .tool-versions`. `.gitlab-ci.yml`'s `oven/bun:<ver>` pins are also verified by `check:docs-versions`, which additionally asserts the two digest-pinned references agree with each other and that the file yields at least one reference to check.
...
- **Claude Code CLI pin guard.** Both images set `CLAUDE_CODE_PATH=/usr/bin/claude`, so the Agent SDK drives the Dockerfile-installed CLI, not its bundled one. `ci.yml` and the GitLab `guards` job run `bun run check:claude-code-version` (`scripts/check-claude-code-version.ts`), which fails unless the `CLAUDE_CODE_VERSION` ARG in both `Dockerfile.orchestrator` and `Dockerfile.daemon` equals the installed SDK's `claudeCodeVersion`. `renovate.json` groups `@anthropic-ai/claude-agent-sdk` and `@anthropic-ai/claude-code` into one PR, but Renovate picks each version independently, so the guard, not the grouping, is what enforces the match. An SDK bump must move the ARG in the same change.
