...
/**
 * Direct workflow dispatch by name. The single entry both triggers use:
 * the `bot:<workflow>` label rail via `dispatchByLabel`, and the mention
 * rail via `command-dispatch.ts routeToHandler`, so one protocol owns the
 * idempotency and refusal semantics for every dispatch.
 *
 * Seven-step protocol: context
